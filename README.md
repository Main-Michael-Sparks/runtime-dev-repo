...
This event surface does not add durable persistence, process-restart recovery, cross-process pub/sub, retained/durable stream-delta storage, or a real `eventLogStoreBackend`. `runtime/bus/actionEventLog/` now defines the contract shape a future event-log store adapter must satisfy, but runtime publication is not wired to durable storage and no database/file backend exists yet. Retained/replayable stream-delta policy remains future work.
