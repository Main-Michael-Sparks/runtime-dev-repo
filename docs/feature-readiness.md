...
actionEventLog contract helpers
  -> future durable-log adapter shape validation only
  -> no runtime persistence wiring, no database/file backend, and no stream-delta durability by default
```
