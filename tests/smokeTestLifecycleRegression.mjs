// smokeTestLifecycleRegression.mjs
//
// Purpose:
// - Current-branch lifecycle regression smoke test.
// - Refactored from the old Init Retry v1 smoke test after configOverride
//   became active in hardware-aware / degraded init retry v1.
//
// Recommended location:
//   tests/smokeTestLifecycleRegression.mjs
//
// Run:
//   node ./tests/smokeTestLifecycleRegression.mjs
//
// Notes:
// - This intentionally spawns isolated child processes so each scenario gets a
//   fresh module/runtime state.
// - Shutdown remains last within any scenario that uses it.
// - Timeout tests intentionally force a failed init attempt and then verify
//   explicit recovery.
...
async function modeInvalidOptionsCurrentContract() {
  ...
  await expectReject(
    ...
    "Unsupported configOverride path: runtime",
  );

  await expectReject(
    "old ambiguous allowGpuLayerFallback option",
    () =>
      initModel({
    ...
    "Unsupported hardwareAware option: allowGpuLayerFallback",
  );

  console.log("[OK] current invalid option checks passed");
...
async function modeRetryTimeoutRecovery() {
  ...
  console.log("[OK] forced default timeout did not crash runtime");
  ...
  console.log("[OK] explicit init recovered after forced default timeout");
...
async function modeLifecycleRegression() {
  logSection("lifecycle regression after hardware-aware init retry changes");
...
async function orchestrator() {
  const modes = [
    "invalid-options-current-contract",
...
async function main() {
  ...
  switch (MODE) {
    case "orchestrator":
      await orchestrator();
      break;
    case "invalid-options-current-contract":
      await modeInvalidOptionsCurrentContract();
      break;
